...
class Seq2SeqAgent(BaseAgent):
    ...
    def generate(self, encoded_instructions, scanId, viewpointId,heading = 0.0, elevation = 0.0):
        ...
        initial_obs = [observe(sim)]
        batch_size = len(initial_obs)
        
        u_t_prev = self.decoder.u_begin.unsqueeze(0)
        ...
        obs = initial_obs
        for t in range(self.episode_len):
            f_t_list = torch.tensor(obs[0]['feature'], device = torch.device('cuda')).unsqueeze(0) # Image features from obs
            all_u_t, is_valid, _ = self._action_variable(obs)
            ...
            #print(a_t[0])
            action_idx = a_t[0].item()
            
            
            adj_loc_ls = obs[0]['adj_loc_list']
            nextViewpointId = adj_loc_ls[action_idx]['nextViewpointId']
            viewIndex = adj_loc_ls[action_idx]['absViewIndex']
            env._navigate_to_location(sim, nextViewpointId, viewIndex)
            ob = observe(sim)
            obs = [ob]
