...
def multi_bleu(multiple_references, hypotheses):
    ...
    print('THE NUMBER OF REFERENCES:', num_refs)
    refss, cands = [], []
    cnt = 0
    for refs, cand in zip(multiple_references, hypotheses):
        if len(refs) == num_refs:
            refss.append(refs)
            cands.append(cand)
        else:
            cnt += 1
    print("%d number of data is removed because of the multi-reference constraint" % cnt)
    multiple_references, hypotheses = refss, cands
